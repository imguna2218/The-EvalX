...
// Removed: use redis::AsyncCommands; // Not directly needed here anymore for pool ops
use serde::Serialize;
use std::env;
use std::net::SocketAddr;
use tokio::signal;
use tracing::{error, info}; // Removed debug, warn for this file for now
// Removed: use std::time::Duration; // Not needed here anymore
...
async fn main() -> Result<()> {
    ...
    if mode == "worker" {
        ...
        start_workers(queue_manager, redis_client, executor).await; 
        signal::ctrl_c().await?;
